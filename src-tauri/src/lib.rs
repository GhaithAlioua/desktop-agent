...
struct DockerStatus {
    ...
    desktop_update_available: Option<bool>,
    error: Option<String>,
    container_count: Option<i32>,
...
fn get_docker_state() -> Arc<Mutex<DockerState>> {
    DOCKER_STATE
...
async fn initialize_docker_monitoring(app_handle: tauri::AppHandle) {
...
async fn start_event_monitoring(
    ...
    while let Some(event_result) = events.next().await {
        match event_result {
            ...
            Err(_e) => {
                // Immediately mark Docker as not running when event stream breaks
                // This ensures we show the restart transition
                {
                    let mut state_guard = state.lock().unwrap();
                    state_guard.status = DockerStatus {
                        is_running: false,
                ...
                notify_status_update(app_handle, state).await;
                break; // Event stream error, connection likely lost
            }
        }
    }
...
#[tauri::command]
async fn subscribe_to_docker_events(app_handle: tauri::AppHandle) -> Result<(), String> {
    let state = get_docker_state();
    let mut receiver = {
        let state_guard = state.lock().unwrap();
...
pub fn run() {
    tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            tauri::async_runtime::spawn(async move {
                initialize_docker_monitoring(app_handle).await;
            });
            Ok(())
        })
